...
const char* password = "kasifsen63";
String hubid = "1";
String serverName = "http://182.176.105.147:4243/api/lls?";
...
void loop() // run over and over
{
  ...
  if (mySerial.available())
  {
    ...
    if(crc == 0)
    {
      ...
      level = message[5]*256 + message[4];
      Serial.print("Level = ");
      Serial.println(level);

      frequency = message[7]*256 + message[6];
      ...
      if(WiFi.status()== WL_CONNECTED)
      {
        WiFiClient client;
        HTTPClient http;
        String serverPath = serverName + "hubid="+hubid+"&level="+String(level)+"&temperature="+String(message[3])+"&frequency="+String(frequency);
